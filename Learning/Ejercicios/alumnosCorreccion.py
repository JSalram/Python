...
while seguir:
    ...
    if opcion == 1:
        for i in range(len(lista)):
            print("{} - {}:{}".format(i+1,lista[i],listaNotas[i])  # Falta paréntesis

    
    elif opcion == 2:
        # Controlar el rango de las notas (0 <= nota <= 10)
        alumno = input("Alumno: ")
...
def ordenaBurbuja(lista,listaNotas):
    for i in range(len(lista)):
        for j in range(lista):  # range(lista) != range(len(lista)) (también falta el -1)
            k=j+1
